		...
		RTDP
{
	...
	public UtilityFunction solve(final MDP mdp) {
		...
		final Set<State> initials = new LinkedHashSet<>(size);
		for ( int i = 0; i < size; i++ ) {
			initials.add(Iterables.get(mdp.getStates(), i));
		}
		return solve(mdp, initials, new UtilityFunctionImpl(mdp.getStates(), Double.MAX_VALUE));
	}
	...
	@Override
	public UtilityFunction solve(final MDP mdp, final Set<State> initials, final long maxDepth) {
		return solve(mdp, initials, emptySet(), maxDepth, new UtilityFunctionImpl(mdp.getStates(), 1.0));
	}

	@Override
	public UtilityFunction solve(final MDP mdp, final Set<State> initials, final UtilityFunction initialValues) {
		return solve(mdp, initials, emptySet(), initialValues);
	}

	@Override
	public UtilityFunction solve(final MDP mdp, final Set<State> initials, final Set<State> goals, final UtilityFunction initialValues) {
		return solve(mdp, initials, goals, Long.MAX_VALUE, initialValues);
	}

	@Override
	public UtilityFunction solve(final MDP mdp, final Set<State> initials, final Set<State> goals, final long maxDepth,
		...
		while ( !convergencyCriteria.hasConverged() ) {
			long depth = 0;
			final Deque<State> visitedStates = new LinkedList<>();
			...
			for ( final Iterator<State> it = visitedStates.descendingIterator(); it.hasNext(); ) {
	...
	private static State chooseNextState(final State s, final Action a, final MDP mdp) {
	...
	private static UtilityFunction backup(final UtilityFunction actualUpper, final State s, final MDP mdp) {
