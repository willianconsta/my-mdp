...
public class PolicyIterationImpl implements PolicyIteration {
	...
	final UtilityFunction function = new UtilityFunctionImpl(
		mdp.getStates());
    ...
    private boolean iteration(final Policy policy,
	    final UtilityFunction function, final MDP mdp) {
	final UtilityFunction evaluatedFunction = evaluator.policyEvaluation(
		policy, function, mdp);
	boolean hasChanged = false;
	for (final State s : mdp.getStates()) {
	    ...
	    double maxValue = 0.0;
	    final Pair<Action, Double> greedyAction = getGreedyActionForState(
		    s, evaluatedFunction, mdp);
	    ...
	    for (final Entry<State, Double> nextStateAndProb : mdp
		    .getPossibleStatesAndProbability(s, policy.getActionFor(s))
		    .entrySet()) {
		policyValue += nextStateAndProb.getValue()
			* evaluatedFunction.getUtility(nextStateAndProb
				.getKey());
	    }
